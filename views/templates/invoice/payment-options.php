...
<?php elseif ( count( $payment_options ) === 1 ) : ?>
	<?php foreach ( $payment_options as $slug => $options ) : ?>
		...
		<?php else : ?>
			<a href="<?php si_payment_link( get_the_ID(), $slug ) ?>" href="<?php esc_url_raw( si_payment_link( get_the_ID(), $slug ) ) ?>" data-slug="<?php esc_attr_e( $slug ) ?>" data-id="<?php the_ID() ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( SI_Controller::NONCE ) ) ?>" class="button primary_button payment_option <?php if ( si_is_cc_processor( $slug ) ) { echo 'cc_processor'; } ?> <?php echo esc_attr( $slug ); ?>"><?php echo esc_html( $payment_string ); ?></a>
		<?php endif ?>
	<?php endforeach ?>
<?php else : ?>
			...
			<?php foreach ( $payment_options as $slug => $options ) : ?>
				<li id="<?php esc_attr_e( $slug ) ?>" class="payment_option">
					<?php if ( isset( $options['purchase_button_callback'] ) ) : ?>
						<?php call_user_func_array( $options['purchase_button_callback'], array( get_the_ID() ) ) ?>
					<?php else : ?>
						<a href="<?php si_payment_link( get_the_ID(), $slug ) ?>" data-slug="<?php esc_attr_e( $slug ) ?>" data-id="<?php the_ID() ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( SI_Controller::NONCE ) ) ?>" class="payment_option <?php if ( si_is_cc_processor( $slug ) ) { echo 'cc_processor'; } ?> <?php echo esc_attr( $slug ) ?>">
