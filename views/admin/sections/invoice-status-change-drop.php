	...
	<span id="status_<?php echo $id ?>">
		<span class="status_change" data-dropdown="#status_change_<?php echo $id ?>">
			...
			<?php if ( $status == SI_Invoice::STATUS_PENDING ): ?>
				<?php printf( '<button class="si_status publish tooltip button current_status" title="%s" disabled><span>%s</span>%s</button>', self::__( 'Pending payment(s)' ), si__('Pending Payment(s)'), $status_change_span ); ?>
			...
			<?php else: ?>
				<?php printf( '<button class="si_status temp tooltip button current_status" title="%s" disabled><span>%s</span>%s</button>', self::__( 'Temp Invoice' ), si__('Temp'), $status_change_span ); ?>
			<?php endif ?>
		</span>
	</span>
