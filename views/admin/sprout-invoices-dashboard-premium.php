<div id="si_dashboard" class="wrap about-wrap">
	...
	<h1><?php printf( self::__( 'Starts with <a href="%s">Sprout Invoices</a>!' ), self::PLUGIN_URL, self::SI_VERSION ); ?></h1>

	<div class="about-text"><?php self::_e('Sprout Invoices is the cornerstone for Sprout Apps. We want it to streamline what could be a very complex workflow of accepting estimates and getting invoices paid. Our mission is to make you a happy user; let us hear from you.') ?></div>
	...
	<div class="welcome_content clearfix">
		<div class="license-overview">
			<h3><?php self::_e('First Things First...') ?></h3>
			<div class="activation_msg clearfix">
				 <p><?php self::_e('A license for Sprout Invoices comes with a 1 year license for support and updates. By activating your license, you can get automatic plugin updates from the WordPress dashboard. Updates provide you with the latest bug fixes and the new features each release brings.') ?></p>
			</div>
			<div class="activation_inputs clearfix">
				<input type="text" name="<?php echo SI_Updates::LICENSE_KEY_OPTION ?>" id="<?php echo SI_Updates::LICENSE_KEY_OPTION ?>" value="<?php echo SI_Updates::license_key() ?>" class="fat-input <?php echo 'license_'.SI_Updates::license_status() ?>" size="40" class="text-input">
				<?php if ( SI_Updates::license_status() != FALSE && SI_Updates::license_status() == 'valid' ): ?>
					<button id="activate_license" class="button button-large" disabled="disabled"><?php self::_e('Activate License') ?></button> 
					<button id="deactivate_license" class="button button-large"><?php self::_e('Deactivate License') ?></button>
				<?php else: ?>
					<button id="activate_license" class="button button-primary button-large"><?php self::_e('Activate License') ?></button>
				<?php endif ?>
