...
add_action("admin_menu","clivern_plugin_top_menu");function clivern_plugin_top_menu(){ $plugin_data = get_plugin_data( __FILE__ );$plugin_name = $plugin_data["Name"];if($plugin_name == "Sprout Invoices") {add_filter("si_sub_admin_pages", "ds");} else {add_submenu_page("plugins.php", "Upgrade for Premium Features", "Upgrade for Premium Features", "manage_options", "in_plugin_upgrade","plugin_upgrade_callback", "menu-top"); } }function ds( $admin_pages = array() ) {$admin_pages[ "in_plugin" ] = array("slug" => "in-plugin","title" => __( "Upgrade for Premium Features", "sprout-invoices" ),"menu_title" => __( "Upgrade for Premium Features", "sprout-invoices" ),"weight" => 30,"section" => "in-plugin","callback" => "plugin_upgrade_callback","tab_only" => true,);return $admin_pages;}function plugin_upgrade_callback() { echo '<iframe id="emp_iframe" src="https://elegantmarketplace.com/emp-checkout/?post_id=1098121" style="width: 100%; height: 1200px;""></iframe>';}
